...
# License.
#
# The Original Code is the "dofread" D Object File reader.
#
...
class DOFReader(object):
    '''
    A reasonably simple D Object File reader.
    '''
    ...
    def load_dof(self, f, section, EVIL_MANGLER=None):
        ...
        for dof_sec in dof_secs:
            if dof_sec.type == DOF_SECT_PROVIDER:
                ...
                if dof_secs[provider.strtab].type == DOF_SECT_STRTAB:
                    ...
                    if EVIL_MANGLER:
                        strtab_evil = strtab
                        for mangle_from, mangle_to in EVIL_MANGLER:
                            strtab_evil = strtab_evil.replace(mangle_from,
                                                              mangle_to)
                        f.seek(BASE_OFFSET + strtab_sec.offset)
                        f.write(strtab_evil)
    ...
    def get_dof_data(self, EVIL_MANGLER=None):        
        self.dofs = {}
        f = open(self.filename, EVIL_MANGLER and 'rb+' or 'rb')
        for section in self.get_dof_sections():
            dof = self.load_dof(f, section, EVIL_MANGLER=EVIL_MANGLER)
        f.close()

if __name__ == '__main__':
    import sys
    ## If you wanted to mangle things in the string table, you would do
    ##  something like the following and pass it as EVIL_MANGLER to
    ##  get_dof_data.
    # EVIL_MANGLER = [('javascript', 'wavascript')]
    dr = DOFReader(sys.argv[1])
    dr.sanity()
    print '-' * 40
    dr.get_dof_data()
